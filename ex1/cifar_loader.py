...
def prepare_cifar_data(dataset_dir: str, cache_dir: str, subsample_fraction: float = 0.1,
...
def ZCA_whitened_images(X_train, X_test, num_components=-1):
    ...
    epsilon = 1e-12
    U_transpose = U.T
    ZCA_mat = np.dot(U[:, :num_components], np.dot(np.diag(1.0 / np.sqrt(S + epsilon)[:num_components]),
                                                   U_transpose[:num_components, :]))  # [M x M]
